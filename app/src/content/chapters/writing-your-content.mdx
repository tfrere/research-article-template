...
---
```
<small className="muted">**Imports** in app/src/content/article.mdx</small>
```mdx
import placeholder from '../assets/images/placeholder.png'
...
| --- | --- | --- | --- |
| title | string | Yes | Main title; supports line breaks with "\n" |
| subtitle | string | No | Shown just below the title |
| description | string | No | Used for SEO/meta description |
| published | string/date | No | e.g., "2025-02-19" or readable date |
| tags | string[] | No | List of keywords |
| authors | `string[] or { name, url?, affiliations? }[]` | No | Affiliation indices refer to the `affiliations` list |
| affiliations | `{ name, url? }[]` | No | Alias: `affiliation` (single or array) |
| doi | string | No | DOI identifier |
| licence | string (HTML allowed) | No | Rendered in footer; HTML supported |
| seoThumbImage | string (URL) | No | Overrides default OpenGraph image |
| tableOfContentsAutoCollapse | boolean | No | Controls TOC auto-collapse |
</Accordion>
