...
export class SVGManager {
  ...
  renderAxes(xTicksForced, yTicksForced, innerWidth, innerHeight, fontFamily, logScaleX, hoverSteps) {
    ...
    this.gAxes.append('g')
      ...
      .call(g => {
        // Remove axis domain line (horizontal bar) but keep ticks
        g.select('.domain').remove();
        g.selectAll('line').style('stroke', 'var(--trackio-chart-axis-stroke)');
    ...
    this.gAxes.append('g')
      ...
      .call(g => {
        // Remove axis domain line (vertical bar) but keep ticks
        g.select('.domain').remove();
        g.selectAll('line').style('stroke', 'var(--trackio-chart-axis-stroke)');
        g.selectAll('text').style('fill', 'var(--trackio-chart-axis-text)')
          .style('font-size', '11px').style('font-family', fontFamily);
